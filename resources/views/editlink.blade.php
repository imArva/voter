...
@section('content')
    ...
    <div class="sm:py-8 sm:ml-64">
        <div class="flex items-center justify-center  mt-14">
            <input id="voting-id" type="hidden" value="{{ Crypt::encryptString($voting->id) }}">
                ...
                <div class="flex flex-col justify-center items-center text-center">
                    <div class="group w-full">
                        <div id="title" contenteditable="true"
                            ...
                            <h1 class="ql-align-center">{!! $voting->title !!}</h1>
                        </div>
                            ...
                            @foreach (json_decode(json_decode($voting->body)) as $key => $item)
                                <label for=""
                                    ...
                                    <div class="flex items-center justify-center w-full h-full p-5">
                                        <label for="dropzone-file-{{ $key }}" class="flex flex-col items-center justify-center border-2 border-gray-300 border-dashed rounded-lg w-full h-full cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                                <div id="file-text-{{ $key }}" class="flex flex-col items-center justify-center @if(!is_null($item->image)) hidden @endif ">
                                                ...
                                                @if ($item->image != "")
                                                    <img id="uploaded-image-{{ $key }}" class="hidden mt-2" />
                                                @else
                                                    <img src="{{ $item->image }}" id="uploaded-image-{{ $key }}" class="mt-2" />
                                                @endif
                                            </div>
                                            <p id="editText-{{ $key }}" class="@if(is_null($item->image)) hidden @endif text-gray-500 font-semibold">Click to change photo</p>
                                            <input id="dropzone-file-{{ $key }}" type="file" class="hidden" onchange="previewImage(event, {{ $key }})" />
                                        </label>
                                    </div> 
                                    ...
                                    <div class="flex flex-col justify-between p-4 w-full leading-normal">
                                        <div class="w-full group">
                                            ...
                                            <div id="t-{{ $key }}" contenteditable="true" contenteditable="true"
                                        ...
                                        <div class="w-full group">
                                            <div id="d-{{ $key }}" contenteditable="true"
